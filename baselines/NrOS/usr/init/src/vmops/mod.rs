...
fn maponly_bencher(cores: usize) {
    ...
    info!("-------Coming into maponly_bencher. This core is {}------------", current_core);
    ...
    info!("bench_duration_secs: {}", bench_duration_secs);
    #[cfg(feature = "latency")] {
        info!("I have latency");
    }
    ...
    s.spawn(
        ...
        move |_| {
            ...
            for core_id in core_ids {
                thandles.push(
                    Environment::thread()
                        .spawn_on_core(Some(maponly_bencher_trampoline), cores as *mut u8, core_id)
