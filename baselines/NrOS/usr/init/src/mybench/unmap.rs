...
pub fn run_test(
    ...
    unsafe {
        region = crt::mem::mmap(
            null_mut(),
            (num_prealloc_pages * PAGE_SIZE * num_threads as i32).try_into().unwrap(),
    ...
    let tot_runs = NUM_MUNMAPS;
    let mut data = thread_data.lock();
    let mut lat_datas = &mut data[0].lat;
...
unsafe extern "C" fn my_worker_thread(arg: *mut u8) -> *mut u8 {
    ...
    if thread_id == 1 {
        unsafe {
            for i in 0..NUM_MUNMAPS {
                VSpace::map_frame(frame_id, base).expect("Map syscall failed");
                let unmap_start = x86::time::rdtsc();
                // info!("[thread {}]: I am going to unmap.", thread_id);
                VSpace::unmap(base, PAGE_SIZE as u64).expect("Unmap syscall failed");
                let unmap_end = x86::time::rdtsc();
                let total_time = get_time_in_nanos(unmap_start, unmap_end);
                // info!("total_time: {}", total_time);
                data.lat.push(total_time as i128);
            }
        }
    } else {
...
pub fn my_test() {
    ...
    entry_point(my_worker_thread, NUM_MUNMAPS);
}
