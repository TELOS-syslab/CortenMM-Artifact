...
def build_userspace(args):
    ...
    print("The args.mods is {}", args.mods)
...
def deploy(args):
    ...
    # shutil.copy2('../../metis/obj/app/nros', esp_path / 'nros')
    # shutil.copy2('../../metis/obj/app/wrmem', esp_path / 'wrmem')
    # shutil.copy2('../../hello', esp_path / 'hello')
    shutil.copy2('../../map', esp_path / 'map')
    ...
    with open(esp_path / 'cmdline.in', 'w') as cmdfile:
        print("args.cmd is {}", args.cmd)
        if args.cmd:
            # cmdfile.write('./kernel {}'.format(args.cmd))
            # cmdfile.write('./kernel log=info ./nros mid=0')
            # cmdfile.write('./kernel log=info ./wrmem mid=0')
            # cmdfile.write('./kernel log=info ./hello mid=0')
            cmdfile.write('./kernel log=info ./map mid=0')
        else:
...
def run_qemu(args):
    ...
    if args.kgdb:
        ...
        qemu_default_args += ['-serial', 'tcp:127.0.0.1:1234,server']
    ...
    if args.qemu_cores and args.qemu_affinity:
        if args.qemu_affinity == "[]":
            ...
            # print("affinity_list1 is {}".format(affinity_list))
        else:
            ...
            # print("affinity_list2 is {}".format(affinity_list))
        ...
        while True:
            try:
                print("str(execution.pid) is {}".format(str(execution.pid)))
                sudo[python3['./qemu_affinity.py',
                             '-k', affinity_list.split(' '), '--', str(execution.pid)]]()
                # with open('script.log', 'w') as log_file:
                #     pass 
                # command = [
                #     'python3', './qemu_affinity.py',
                #     '-k', affinity_list,  # 如果 affinity_list 是字符串，分割后再拼接为命令行参数
                #     '--', str(execution.pid)
                # ]
