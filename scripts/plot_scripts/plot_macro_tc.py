...
def main():
    ...
    gs = GridSpec(4, 3, width_ratios=[1.5, 1, 1])

    # Create a dict for the axes
    axs = {}

    # First column (no split)
    axs[0, 0, 0] = fig.add_subplot(gs[0:2, 0])
    axs[1, 0, 0] = fig.add_subplot(gs[2:4, 0])

    # Column 1 and 2 (split rows 0 and 1)
    axs[0, 1, 0] = fig.add_subplot(gs[0, 1])  # top
    axs[0, 2, 0] = fig.add_subplot(gs[0, 2])  # top
    axs[1, 1, 0] = fig.add_subplot(gs[2, 1])  # top
    axs[1, 2, 0] = fig.add_subplot(gs[2, 2])  # top

    # Bottom parts of the split rows
    axs[0, 1, 1] = fig.add_subplot(gs[1, 1])  # bottom of axs[0, 1]
    axs[0, 2, 1] = fig.add_subplot(gs[1, 2])  # bottom of axs[0, 2]
    axs[1, 1, 1] = fig.add_subplot(gs[3, 1])  # bottom of axs[1, 1]
    ...
    axs[0, 0, 0].legend(handles, labels, loc='upper center', bbox_to_anchor=(1.1, 1.48), ncol=4, fontsize=6)
